document.addEventListener("DOMContentLoaded", function () {
    ...
    const aboutTexts = [
    {
        ...
        desc: "after 051은 부산의 바다에서 시작된 헤어·바디 리추얼 브랜드입니다. 바다 위로 번지는 빛, 안개처럼 흐려지는 경계, 하루의 끝에 남는 고요함에서 우리의 감각은 출발합니다."
    },
    {
        count: "02 / 051",
    ...
    },
    {
        ...
        desc: "씻고 바르는 일은 단순한 케어가 아니라 하루의 감각을 천천히 정리하는 리추얼이 될 수 있습니다. after 051은 헤어와 바디에 남는 향과 텍스처를 통해 평범한 일상에 조용한 휴식을 제안합니다."
    },
    {
        count: "06 / philosophy",
        title: "For your<br>quiet rest.",
        desc: "우리는 강한 인상보다 오래 머무는 잔향을 믿습니다. 부산의 바다에서 발견한 빛, 안개, 파도, 깊이를 일상의 가장 가까운 감각으로 옮깁니다. 당신의 평온한 휴식을 위해, after 051."
    }
];
    ...
    gsap.to(visualTrack, {
        yPercent: -100 * (visualSlides.length - 1),
        ease: "none",
        scrollTrigger: {
            trigger: "#about_story",
            start: "top top",
            end: "+=" + (window.innerHeight * (visualSlides.length - 1) * 0.1),
            pin: ".about_pin",
            scrub: 1,
            onUpdate: function (self) {
                const index = Math.round(self.progress * (aboutTexts.length - 1));
                changeText(index);
            }
        }
    });
